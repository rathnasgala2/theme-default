...
Own the fallback presentation theme (`@rathnasgala2/theme-default`), the
complete-when-no-theme-is-selected default of the S2 theme family. This
repository never owns rendering, deployment, provider credentials, or any
other theme's presentation bytes (S2 brief §4; task packet S2-T13).
...
`theme-zebra`, S2-T14), the reusable release workflow or this repository's
own workflow caller (S2-T11, infra, REMOTE-ONLY), Playwright/axe-core
browser conformance (S2-T22), template rendering logic, or `publish`
kernel/adapter logic.
